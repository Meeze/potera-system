    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (label.equalsIgnoreCase("money")) {
            ...
            if (args.length == 2 || args.length > 3) {
                ...
                if (player.hasPermission("potera.money.edit"))
                    player.sendMessage(
                            StringDefaults.PREFIX + "§cVerwendung: §7/" + label + " <add|remove|set> <Spieler> <Münzen>");
                return true;
            }
            ...
            if (args.length == 0) {
                ...
                player.sendMessage(
                        StringDefaults.PREFIX + "§eDeine Münzen§8: §7" + Util.formatNumber(
                                user.getUserCurrency().getMoney()) + "$");
                return true;
            }
            ...
            if (args.length == 1) {
                ...
                player.sendMessage(
                        StringDefaults.PREFIX + "§e" + target.getName() + "'s Münzen§8: §7" + Util.formatNumber(
                                user.getUserCurrency().getMoney()) + "$");
                return true;
            }
            ...
            if (!args[0].equalsIgnoreCase("add") && !args[0].equalsIgnoreCase("remove") && !args[0].equalsIgnoreCase(
                    "set")) {
                player.sendMessage(StringDefaults.PREFIX + "§cVerwendung: §7/" + label + " [Spieler]");
            ...
            if (target == null) {
                UUIDFetcher.getUUID(args[1], uuid -> {
                    ...
                    if (args[0].equalsIgnoreCase("remove") && user.getUserCurrency().getMoney() - amount < 0L) {
                        player.sendMessage(StringDefaults.PREFIX + "§cDer Spieler hat nicht soviel Geld.");
                        return;
                    }

                    if (args[0].equalsIgnoreCase("add")) {
                        user.getUserCurrency().addMoney(amount);
                        player.sendMessage(
                                StringDefaults.PREFIX + "§eDu hast dem Geld von §7" + name + " " + Util.formatNumber(
                                        amount) + "$ §ehinzugefügt.");
                    } else if (args[0].equalsIgnoreCase("remove")) {
                        user.getUserCurrency().removeMoney(amount);
                        player.sendMessage(
                                StringDefaults.PREFIX + "§eDu hast dem Geld von §7" + name + " " + Util.formatNumber(
                                        amount) + "$ §eentfernt.");
                    } else {
                        user.getUserCurrency().setMoney(amount);
                        player.sendMessage(
                                StringDefaults.PREFIX + "§eDas Geld von §7" + name + " §ewurde auf " + Util.formatNumber(
                                        amount) + "$ §egesetzt.");
                    }
                });
                return true;
            ...
            if (args[0].equalsIgnoreCase("add")) {
                ...
                if (target != player)
                    player.sendMessage(
                            StringDefaults.PREFIX + "§eDu hast dem Geld von §7" + target.getName() + " " + Util.formatNumber(
                                    amount) + "$ §ehinzugefügt.");
                target.sendMessage(StringDefaults.PREFIX + "§eDeinem Geld wurden §7" + Util.formatNumber(
                        amount) + "$ §ehinzugefügt.");
            } else if (args[0].equalsIgnoreCase("remove")) {
                user.getUserCurrency().removeMoney(amount);
                if (target != player)
                ...
                if (target != player)
                    player.sendMessage(
                            StringDefaults.PREFIX + "§eDas Geld von §7" + target.getName() + " §ewurde auf " + Util.formatNumber(
                                    amount) + "$ §egesetzt.");
                target.sendMessage(StringDefaults.PREFIX + "§eDein Geld wurde auf §7" + Util.formatNumber(
                        amount) + "$ §egesetzt.");
            }
        }
        ...
        if (label.equalsIgnoreCase("pay")) {
            ...
            if (pUser.getUserCurrency().getMoney() < amount) {
                player.sendMessage(StringDefaults.PREFIX + "§cDu hast nicht soviel Geld.");
                return true;
            }

            pUser.getUserCurrency().removeMoney(amount);
            tUser.getUserCurrency().addMoney(amount);


            player.sendMessage(
                    StringDefaults.PREFIX + "§eDu hast §7" + target.getName() + " §eerfolgreich §7" + Util.formatNumber(
                            amount) + "$ §egesendet.");
            target.sendMessage(StringDefaults.PREFIX + "§eDu hast §7" + Util.formatNumber(
                    amount) + "$ §evon §7" + player.getName() + " §eerhalten.");
        }
