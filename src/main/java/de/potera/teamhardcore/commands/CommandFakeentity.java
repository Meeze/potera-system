...
public class CommandFakeentity implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (!player.hasPermission("potera.fakeentity")) {
        ...
        if (args.length == 3) {
            if (args[0].equalsIgnoreCase("create")) {
                ...
                if (type == null) {
                    ...
                    player.sendMessage(
                            StringDefaults.PREFIX + "§cVerfügbare Typen: " + sb.substring(0, sb.length() - 4));
                    return true;
                }
                ...
                if (Main.getInstance().getFakeEntityManager().isEntityExists(name)) {
                    player.sendMessage(
                            StringDefaults.PREFIX + "§cEs existiert bereits ein FakeEntity mit diesem Namen.");
                    return true;
                }
            ...
            if (args[0].equalsIgnoreCase("line") && args[1].equalsIgnoreCase("remove")) {
                ...
                if (args.length == 4) {
                    try {
                        ...
                        player.sendMessage(
                                StringDefaults.PREFIX + "§cBitte gebe eine gültige Position ein. (1-" + lines.size() + ")");
                        return true;
                    }
                }
                ...
                player.sendMessage(
                        StringDefaults.PREFIX + "§eDie " + ((pos == -1) ? "letzte Zeile" : ("Zeile " + ++pos)) + " §edes Nametags wurde gelöscht.");
                return true;
            }
        }
        ...
        if (args.length >= 4) {
            if (args[0].equalsIgnoreCase("line") && args[1].equalsIgnoreCase("add")) {
                ...
                entity.setNametag(lines);
                player.sendMessage(
                        StringDefaults.PREFIX + "§eEs wurde" + ((pos == -1) ? "" : (" an der Stelle §7" + ++pos + "§e")) + " eine Zeile des Nametags hinzugefügt.");
                return true;
            }
        }
        ...
        if (args.length >= 5) {
            if (args[0].equalsIgnoreCase("option")) {
                ...
                if (state == null) {
                    ...
                    player.sendMessage(
                            StringDefaults.PREFIX + "§cVerfügbare Typen: §7" + sb.substring(0, sb.length() - 6));
                    return true;
                }
                ...
                FakeEntityOptionBase option = Main.getInstance().getFakeEntityManager().getOption(state, category,
                        optionName);
                ...
                if (set) {
                    ...
                    try {
                        handlingOption = (FakeEntityOptionImpl) optionClass.getConstructor(
                                new Class[]{FakeEntityOptionBase.class, FakeEntity.class, String[].class}).newInstance(
                                new Object[]{option, entity, params});
                    ...
                    if (!handlingOption.validateApplicableOption()) {
                        player.sendMessage(
                                StringDefaults.PREFIX + "§cDiese Option ist für den angegebenen Mob nicht nutzbar.");
                        return true;
                    }
            ...
            if (args[0].equalsIgnoreCase("line") && args[1].equalsIgnoreCase("set")) {
                ...
                player.sendMessage(
                        StringDefaults.PREFIX + "§eEs wurde die Zeile §7" + ++pos + " §edes Nametags editiert.");
                return true;
            }
        }
    ...
    private void sendHelp(Player p, String label) {
        ...
        p.sendMessage(
                StringDefaults.PREFIX + "§cVerwendung: §7/" + label + " option <Entity> <set|remove> <Typ> <Option> [Parameter]");
