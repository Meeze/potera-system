...
import de.potera.rysefoxx.menubuilder.interfaces.ItemCallback;
...
public class InventoryMenuBuilder extends MenuBuilder {
    ...
    public InventoryMenuBuilder withEventHandler(@Nonnull InventoryEventHandler eventHandler) {
        try {
            MenuBuilderPlugin.inventoryListener.registerEventHandler(this, eventHandler);
    ...
    public InventoryMenuBuilder onInteract(@Nonnull InventoryMenuListener listener, @Nonnull ClickType... actions) {
        ...
        try {
            MenuBuilderPlugin.inventoryListener.registerListener(this, listener, actions);
        } catch (IllegalArgumentException e) {
            throw e;
        }
        return this;
    ...
    public InventoryMenuBuilder withItem(@Nonnegative final int slot, @Nonnull final ItemStack item, @Nonnull final ItemListener listener, @Nonnull ClickType... actions) {
        withItem(slot, item);
        onInteract((player, action, slot_) -> {
            if (slot_ == slot) {
    ...
    @Override
    public void dispose() {
        MenuBuilderPlugin.inventoryListener.unregisterAllListeners(getInventory());
    }

    public void unregisterListener(InventoryMenuListener listener) {
        try {
            MenuBuilderPlugin.inventoryListener.registerListener(this, listener, ALL_CLICK_TYPES);
        } catch (IllegalArgumentException e) {
            throw e;
        }
    }
